...
class TileHandleA1 extends TileHandle {
  constructor(x,y,config_str){
    ...
    this.tile_id = Tilemap.TILE_ID_A1 + this.tile_index * 48;

    // A1 私有属性
    this.is_auto_tile = TileHandle.AUTO_A1_TILE_INDEX.includes(this.tile_index);
    this.is_special_tile = TileHandle.SPECIAL_A1_TILE_ID.includes(this.tile_index);
  }
  ...
  isCurrentTileA1ByIndex(index,tile_id,layer){
  ...
  isCurrentTileA1(x,y,tile_id,layer){
  ...
  getTileIndexByTileId(tile_id){
    return parseInt((tile_id - Tilemap.TILE_ID_A1) / 48);
  };
  ...
  checkIsAutoTileByPos(x,y,layer){
    ...
    return TileHandle.AUTO_A1_TILE_INDEX.includes(tile_index);
  };
  ...
  checkIsSpecialTileByTileId(tile_id){
    const tile_index = this.getTileIndexByTileId(tile_id);
    const is_special_tile = TileHandle.SPECIAL_A1_TILE_ID.includes(tile_index);
  ...
  checkIsSpecialTileByTilePos(tile_x,tile_y,layer){
    ...
    const tile_id = Tilemap.TILE_ID_A1 + start_tile_index * 48;
    const tile_index = this.getTileIndexByTileId(tile_id);
    const is_special_tile = TileHandle.SPECIAL_A1_TILE_ID.includes(tile_index);
    return is_special_tile;
  };
  ...
  drawTileByPos(x,y){
    ...
    if (tile_id_layer_0 && tile_id_layer_0 !== 0){
      const start_tile_id = tile_id_layer_0 - (tile_id_layer_0 - Tilemap.TILE_ID_A1) % 48;
      ...
      if (is_special_tile){
        this.drawTileCore(x, y,Tilemap.TILE_ID_A1,is_auto_tile,0);
        this.drawTileCore(x, y,start_tile_id,is_auto_tile,1);
      } else {
        this.drawTileCore(x, y,start_tile_id,is_auto_tile,0);
      }
    }
    if (tile_id_layer_1 && tile_id_layer_1 !== 0){
      const start_tile_id = tile_id_layer_1 - (tile_id_layer_1 - Tilemap.TILE_ID_A1) % 48;
      const is_special_tile = this.checkIsSpecialTileByTileId(start_tile_id);
      const is_auto_tile = this.checkIsAutoTileByTileId(start_tile_id);
      if (is_special_tile){
        this.drawTileCore(x, y,Tilemap.TILE_ID_A1,is_auto_tile,0);
  ...
  drawCurrentPosTile(){
    ...
    if (this.is_special_tile){
      this.drawTileCore(this.x, this.y, Tilemap.TILE_ID_A1, this.is_auto_tile, 0);
