...
const AppBodyInit = (props) => {
  ...
  function createTilemapA1(){
    ...
    groups.forEach((arr,index) => {
      arr.forEach((tile_name,tilename_index) => {
        ...
        render_tilemaps["A"].push(createTileEle(image_config,x,y))
      });
    });
  }
  ...
  function getCurrentTilemapSets(){
    return render_tilemaps[App.dprops.project.session.current_tile_set];
  }
      ...
      <div id="container-tool-area">
        <div id="container-tool-tileset-wrapper" class="area-border-color">
          ...
          <div id="container-tool-tileset-layer-tabs" class="inline-flex flex-row flex-row-center">
            <a class={App.dprops.project.session.current_tile_set === 'A' ? 'active' : ''}>A</a>
            <a class={App.dprops.project.session.current_tile_set === 'B' ? 'active' : ''}>B</a>
            <a class={App.dprops.project.session.current_tile_set === 'C' ? 'active' : ''}>C</a>
            <a class={App.dprops.project.session.current_tile_set === 'B' ? 'active' : ''}>R</a>
          </div>
        </div>
